...
async def queryWXDLLM(request: queryLLMElserRequest, api_key: str = Security(get_api_key)):
    ...
    if not llm_instructions:
        llm_instructions = "You are a helpful, respectful, and honest assistant. Always answer as helpfully as possible, while being safe. Be brief in your answers. Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous, or illegal content. Please ensure that your responses are socially unbiased and positive. If a question does not make any sense or is not factually coherent, explain why instead of answering incorrectly. If you do not know the answer to a question, please do not share false information. Generate the next agent response by answering the question. You are provided with several documents with titles. If the answer comes from different documents please mention all possibilities. If you cannot base your answer on the given document, please state that you do not have an answer.\nDocumentation:\n{context_str}\n\nQuestion:\n{query_str}.\n\nEducated response of 400 words or less:"
